...
class InstanceSwitchAuthInterceptor(private val accountSession: AccountSession) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        return if (originalRequest.url.host == MastodonApi.PLACEHOLDER_DOMAIN) {
            ...
            if (instanceHeader != null) {
                ...
                if (accountSession.token != null) {
                    val accessToken = accountSession.token.accessToken
                    if (accessToken.isNotEmpty()) {
                        // use domain of current account
                        builder.url(swapHost(originalRequest.url, accountSession.domain))
