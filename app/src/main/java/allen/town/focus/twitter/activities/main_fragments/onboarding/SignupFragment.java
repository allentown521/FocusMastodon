...
import allen.town.focus.twitter.api.MastodonDetailedErrorResponse;
import allen.town.focus.twitter.api.requests.accounts.RegisterAccount;
...
import allen.town.focus.twitter.api.session.AccountActivationInfo;
...
public class SignupFragment extends ToolbarFragment{
					...
					Jsoup.parseBodyFragment(s).body().traverse(new NodeVisitor(){
						...
						public void tail(Node node, int depth){
							if(node instanceof Element){
								ssb.setSpan(new LinkSpan("", SignupFragment.this::onGoBackLinkClick, LinkSpan.Type.CUSTOM, null,false,"",""), spanStart, ssb.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
	...
	private void showProgressDialog(){
		if(progressDialog==null){
			progressDialog=AccentProgressDialog.show(getContext(),getString(R.string.loading));
