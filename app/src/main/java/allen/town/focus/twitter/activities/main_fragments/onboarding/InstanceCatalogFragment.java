...
import allen.town.focus.twitter.api.MastodonErrorResponse;
import allen.town.focus.twitter.api.requests.instance.GetInstance;
...
abstract class InstanceCatalogFragment extends BaseRecyclerFragment<CatalogInstance>{
	...
	protected void showProgressDialog(){
		instanceProgressDialog=AccentProgressDialog.show(getActivity(),getString(R.string.loading_instance));
	...
	protected void loadInstanceInfo(String _domain, boolean isFromRedirect){
		...
		loadingInstanceRequest.setCallback(new Callback<>(){
			...
			public void onSuccess(Instance result){
				...
				((Instance)result).uri=domain; // needed for instances that use domain redirection
				instancesCache.put(domain, (Instance)result);
				if(instanceProgressDialog!=null){
					instanceProgressDialog.dismiss();
					instanceProgressDialog=null;
					proceedWithAuthOrSignup((Instance)result);
				}
				if(Objects.equals(domain, currentSearchQuery) || Objects.equals(currentSearchQuery, redirects.get(domain)) || Objects.equals(currentSearchQuery, redirectsInverse.get(domain))){
					...
					if(!found){
						CatalogInstance ci= ((Instance)result).toCatalogInstance();
			...
			public void onError(ErrorResponse error){
				loadingInstanceRequest=null;
				if(!isFromRedirect && error instanceof MastodonErrorResponse && ((MastodonErrorResponse)error).httpStatus==404){
				...
				if(fakeInstance!=null){
					...
					if(filteredData.size()>0 && filteredData.get(0)==fakeInstance){
						if(list.findViewHolderForAdapterPosition(1) instanceof BindableViewHolder<?>){
							((BindableViewHolder<?>)list.findViewHolderForAdapterPosition(1)).rebind();
						}
					}
				}
			}
	...
	private void showInstanceInfoLoadError(String domain, Object error){
		if(instanceProgressDialog!=null){
			...
			new AccentMaterialDialog(getActivity(),-1)
