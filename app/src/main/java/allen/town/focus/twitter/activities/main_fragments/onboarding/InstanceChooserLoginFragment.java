...
import allen.town.focus.twitter.api.requests.catalog.GetCatalogInstances;
...
public class InstanceChooserLoginFragment extends InstanceCatalogFragment{
	...
	public void onCreate(Bundle savedInstanceState){
		...
		dataLoaded();
		setTitle(R.string.login_to_twitter);
	...
	private void onButtonClick(View v){
		...
		Nav.go(getActivity(), InstanceCatalogSignupFragment.class, extras);
	}
	@Override
	protected RecyclerView.Adapter getAdapter(){
		...
		headerView.findViewById(R.id.btn_get_started).setOnClickListener(this::onButtonClick);
			...
			public void onTextChanged(CharSequence s, int start, int before, int count){
				...
				if(s.length()>0){
					...
					if(filteredData.size()>0 && filteredData.get(0)==fakeInstance){
						if(list.findViewHolderForAdapterPosition(1) instanceof InstanceViewHolder){
							((InstanceViewHolder)list.findViewHolderForAdapterPosition(1)).rebind();
						}
					}
			...
			if(chosenInstance!=null){
				...
				if(idx!=-1){
					...
					for(int i=0;i<list.getChildCount();i++){
						...
						if(holder.getAbsoluteAdapterPosition()==mergeAdapter.getPositionForAdapter(adapter)+idx && holder instanceof InstanceViewHolder){
