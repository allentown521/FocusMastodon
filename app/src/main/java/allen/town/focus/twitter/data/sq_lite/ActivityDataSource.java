...
import allen.town.focus.twitter.utils.FavoriterUtils;
...
public class ActivityDataSource {
    ...
    public synchronized String insertRetweeters(Status status, int account, boolean isSecondAccount) {
        ...
        if (retweetCountInDb != -1 && retweetCountInDb < status.getRetweetCount()) {
            ...
            ContentValues values = getRetweeterContentValues(status, account, isSecondAccount);
            if (values != null) {
                try {
                    database.update(
    ...
    public ContentValues getMentionValues(Status status, int account) {
        ...
        values.put(ActivitySQLiteHelper.COLUMN_PIC_URL, media);
        values.put(ActivitySQLiteHelper.COLUMN_USERS, JsonHelper.toJSONString(status.getUserMentionEntities()));
    ...
    public ContentValues getFavoriteValues(Status status, int account) {
        ...
        try {
            ...
            List<User> userList = new FavoriterUtils().getFavoriters(id,null);
    ...
    public ContentValues getRetweeterContentValues(Status status, int account, boolean isSecondAccount) {

        try {
            List<User> users = UserJSONImplMastodon.createPagableUserList(
                    isSecondAccount ?
                            new GetStatusReblogs(status.getId() + "", null, null, 80).execSecondAccountSync()
                            : new GetStatusReblogs(status.getId() + "", null, null, 80).execSync());
    ...
    public String buildUserList(List<User> users) {
        List<UserMentionEntityJSONImplMastodon> userMentionEntityJSONImplMastodonList = new ArrayList<>();
        for (User u : users) {
            userMentionEntityJSONImplMastodonList.add(new UserMentionEntityJSONImplMastodon(u.getName(), u.getScreenName(), u.getId(), u.getURL()));
        }
        return JsonHelper.toJSONString(userMentionEntityJSONImplMastodonList);
    }
